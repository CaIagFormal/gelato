...
public class C_Pedidos {
    ...
    public String getPedidos(HttpSession session, Model model) {
        ...
        model.addAttribute("pedidos_preparados",pedidos.get(M_Ticket.StatusCompra.PREPARADO));
    ...
    @ResponseBody
    public M_RespostaTexto cancelarPedidoVendedor(@RequestParam("id") String id) {
        M_RespostaTexto m_respostaTexto = new M_RespostaTexto();
        ...
        if (m_ticket==null) {
            m_respostaTexto.setSucesso(false);
            m_respostaTexto.setMensagem("Não foi possível cancelar o pedido devido a um erro no banco de dados.");
            return m_respostaTexto;
        }
    ...
    /**
     * Avança o status do pedido para próxima etapa como denominado em {@link com.tcc.gelato.model.produto.M_Ticket.StatusCompra}
     * @return Div do pedido cancelado
     */
    @PostMapping("/pedidos/avancar")
    @ResponseBody
    public M_RespostaTexto avancarPedidoVendedor(@RequestParam("id") String id) {
        M_RespostaTexto m_respostaTexto = new M_RespostaTexto();
        ...
        if (m_ticket.getStatus() == M_Ticket.StatusCompra.CARRINHO) {
            m_respostaTexto.setMensagem("O pedido não pode ser avançado quando está no carrinho");
            m_respostaTexto.setSucesso(false);
            return m_respostaTexto;
        }

        if (m_ticket.getStatus() == M_Ticket.StatusCompra.ENTREGUE) {
            m_respostaTexto.setMensagem("O pedido não pode ser avançado quando já foi entregue");
            m_respostaTexto.setSucesso(false);
            return m_respostaTexto;
        }
        ...
        if (m_ticket.getStatus() == M_Ticket.StatusCompra.CANCELADO) {
            m_respostaTexto.setMensagem("O pedido não pode ser avançado quando já foi cancelado");
            m_respostaTexto.setSucesso(false);
            return m_respostaTexto;
        }

        m_ticket = s_ticket.avancarTicket(m_ticket);
        if (m_ticket==null) {
            m_respostaTexto.setSucesso(false);
            m_respostaTexto.setMensagem("Não foi possível avançar o pedido devido a um erro no banco de dados.");
            return m_respostaTexto;
        }

        M_ViewPedido m_viewPedido = s_ticket.getPedidoFromTicket(m_ticket);
        if (m_viewPedido==null) {
            m_respostaTexto.setMensagem("Pedido foi avançado para "+m_ticket.getStatus().toString().toLowerCase(Locale.ROOT)+
                    " mas houve um erro atualizando a tela.<br>Recarregue a página para atualizar seu pedido.");
            m_respostaTexto.setSucesso(false);
            return m_respostaTexto;
        }
